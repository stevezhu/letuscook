...
/**
 * A GuestCapture extended with a local-only `captureState`.
 * This state ('offline') is injected at runtime and intentionally NOT stored
 * in AsyncStorage to save space and simplify the schema.
 */
...
/**
 * The result of an attempt to add a guest capture.
 * We enforce a limit to prevent uncontrolled local storage growth.
 */
export type AddGuestCaptureResult =
  | { status: 'ok'; capture: GuestCapture }
  | { status: 'LIMIT_REACHED' };

/** Maximum number of captures a guest can store locally before being forced to sign in. */
