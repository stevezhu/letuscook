...
A separate topics table forces you to pick *one* category. Nodes + edges naturally support multi-dimensional relationships.
...
3. Ask LLM: "Given this content and these similar nodes, what organizing nodes should this link to? Create new virtual nodes if needed."
...
5. Create edges: content node →[categorized_as] organizing node(s)
6. Hybrid confidence: auto-create edge if confidence > 0.8, suggest for review if lower
7. Check if any virtual nodes should be promoted (threshold met, or capture content matches)
...
Same as Option A but with richer edge metadata to capture *why* things are related:
...
**Pros:** More nuanced relationships, better for rediscovery ("show me things related to React *because of* JavaScript")
...
**No new table needed.** The activity feed is a *view* over existing data (captures + edges).
