...
_(Note: The optional ESLint rule to restrict `query`/`mutation` imports from `_generated/server` was intentionally skipped, as there isn't a dedicated ESLint configuration file for the `assistant-convex` package, and adding it to the root monorepo config would unnecessarily impact the global ruleset)._
